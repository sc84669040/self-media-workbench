...
from topic_packet_service import create_topic_packet, get_topic_packet_detail, run_topic_search
...
from runtime_config import config as root_config, event_radar_db_path, sample_vault_path, service_host, service_port
...
STUDIO_NEXT_HTML_PATH = Path(__file__).with_name("studio_next.html")
...
EVENT_RADAR_DB_PATH = event_radar_db_path()
...
NIGHTHAWK_ONLY_MODE = BASE_DIR.name == "content-search-layer"


@dataclass
class AppConfig:
    host: str = service_host("content_search_layer")
    port: int = service_port("content_search_layer", 8787)
...
def _is_create_route(path: str) -> bool:
    return path.startswith("/create") or path.startswith("/api/create")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):  # noqa: N802
        ...
        if NIGHTHAWK_ONLY_MODE and _is_create_route(path):
        ...
        if path in ["/create", "/create/studio", "/create/studio.html", "/create-next", "/create/next", "/create/next.html"]:
    ...
    def do_POST(self):  # noqa: N802
        if NIGHTHAWK_ONLY_MODE and self.path.startswith("/api/create"):
            self._send_json(404, {"ok": False, "message": "Not Found"})
            return
...
def list_topics(sort_by: str = "overall", page: int = 1, limit: int = 10, keyword: str = "") -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            return {"items": [], "total": 0, "page": 1, "page_size": page_size, "total_pages": 1, "sort_by": sort_by, "keyword": keyword_text}
    ...
    try:
        ...
        if str(topic.get("topic_type") or "").strip() == "article_theme_v2" and has_topic_articles:
            ...
            for row in article_rows:
                ...
                item["content_excerpt"] = _trim_text(item.get("content") or "", 220)
...
def main():
    ...
    parser.add_argument("--host", default=service_host("content_search_layer"))
    parser.add_argument("--port", type=int, default=service_port("content_search_layer", 8787))
