...
class CreationWorkspaceService:
    ...
    def _serialize_bundle(self, task_id: str) -> dict[str, Any]:
        ...
        retrieval_batch = self.store.latest_by_task("retrieval_batches", task_id)
    ...
    def _packet_retrieval_result(
        ...
        return {
            ...
            "text": body_text or str(digest.get("text_excerpt") or article.get("summary") or "").strip(),
            "recommend_full_fetch": "" if body_text else "yes",
            "matched_handle": "",
            "author": "",
            ...
            "raw": {
                ...
                "body_fetch_ok": bool(body_text),
                "body_status": "ready" if body_text else "missing",
    ...
    def get_retrieval_batch(self, task_id: str) -> dict[str, Any] | None:
        return self.store.latest_by_task("retrieval_batches", task_id)
        ...
        retrieval_batch["results"] = [
            {
                ...
                "recommend_full_fetch": "" if item.get("body_ready") else "body_missing",
                "decision": "keep",
                "decision_reason": "用户从 NightHawk 正文池手动保留",
                "classification": "primary" if int(item.get("raw_item_id") or 0) in primary_raw_item_ids else "secondary",
                "classification_reason": "主资料" if int(item.get("raw_item_id") or 0) in primary_raw_item_ids else "辅助资料",
                "raw": {
                    "body_fetch_ok": bool(item.get("body_ready")),
