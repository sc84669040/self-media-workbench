...
class RuleConfig:
    ...
    x_short_text_penalty_multiplier: float
...
def load_rules() -> tuple[TelegramConfig, KnowledgeBaseConfig, RuleConfig]:
    ...
    return (
        ...
        RuleConfig(
            ...
            short_text_penalty=float(rules.get("short_text_penalty", 0.6)),
            x_short_text_penalty_multiplier=float(rules.get("x_short_text_penalty_multiplier", 0.5)),
        ),
    )
...
def compute_score(item: sqlite3.Row, conn: sqlite3.Connection, rules: RuleConfig) -> AlertDecision:
    ...
    if len(cleaned_text) < max(1, rules.min_text_len_for_strong):
        short_text_penalty = max(0.0, rules.short_text_penalty)
        if platform == "x":
            short_text_penalty *= max(0.0, rules.x_short_text_penalty_multiplier)
