...
def test_creation_api_prefers_local_fts_for_topic_retrieval(monkeypatch, tmp_path):
    ...
    store.upsert_content_object(
        {
            ...
            "body_text": "Hermes 的价值，在于把检索、证据、写作和热点接入编排成稳定工作流。",
            "body_ready": True,
    ...
    store.upsert_content_object(
        {
            "object_uid": "OBJ-NOTE-001",
            "source_kind": "source_note",
            "platform": "knowledge_base",
            "source_ref": "note://workflow",
            "title": "内容生产链路笔记",
            "summary": "说明为什么编排层比单次提示词更稳。",
            "body_text": "当主题、检索、证据和写作拆开后，创作链路才开始稳定。",
            "body_ready": True,
            "published_at": "2026-04-20T10:00:00+08:00",
            "source_name": "知识库",
            "related_topics": ["内容生产链路"],
        }
    )
    ...
    def fail_external_search(_payload: dict) -> dict:
        raise AssertionError("creation retrieval should prefer local FTS when it is available")
    ...
    assert run_payload["retrieval_batch"]["topic_intent"]["entities"] == ["Hermes"]
    assert run_payload["retrieval_batch"]["results"]
    assert run_payload["retrieval_batch"]["results"][0]["source_id"] == "OBJ-HERMES-001"
    assert run_payload["retrieval_batch"]["results"][0]["content_type"] == "analysis_card"
    assert "Hermes" in run_payload["retrieval_batch"]["results"][0]["title"]
