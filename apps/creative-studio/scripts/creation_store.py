...
class CreationStore:
    def __init__(self, data_root: str | Path):
        ...
        self.article_drafts_dir = self.data_root / "article_drafts"
        self.article_drafts_dir.mkdir(parents=True, exist_ok=True)

    def _path(self, object_type: str) -> Path:
        return self.data_root / f"{object_type}.jsonl"

    def _rewrite_without(self, object_type: str, predicate) -> None:
    ...
    def delete(self, object_type: str, item_id: str) -> None:
        normalized = str(item_id or "").strip()
        if not normalized:
            return
        self._rewrite_without(object_type, lambda item: str(item.get("id") or "").strip() == normalized)

    def delete_by_task(self, object_type: str, task_id: str) -> None:
        normalized = str(task_id or "").strip()
        if not normalized:
            return
        self._rewrite_without(object_type, lambda item: str(item.get("creation_task_id") or "").strip() == normalized)

    def delete_writer_packets(self, task_id: str) -> None:
        normalized = str(task_id or "").strip()
        if not normalized:
            return
        target_dir = self.writer_packets_dir / normalized
        if target_dir.exists():
            shutil.rmtree(target_dir, ignore_errors=True)

    def delete_article_drafts(self, task_id: str) -> None:
        normalized = str(task_id or "").strip()
        if not normalized:
            return
        target_dir = self.article_drafts_dir / normalized
        if target_dir.exists():
            shutil.rmtree(target_dir, ignore_errors=True)
    ...
    def write_article_draft(self, draft_id: str, task_id: str, markdown: str) -> str:
        target_dir = self.article_drafts_dir / task_id
        target_dir.mkdir(parents=True, exist_ok=True)
        path = target_dir / f"{draft_id}.md"
        path.write_text(str(markdown or ""), encoding="utf-8")
        return str(path)

    def read_article_draft(self, article_path: str | Path) -> str:
        path = Path(str(article_path or "").strip())
        if not path.exists():
            return ""
